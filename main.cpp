...
std::vector<std::string> words {"a", "ability", "able", "about", "above", "accept", "according", "account", "across", "act", "action",
...
std::string passGen(int user_number) {
    std::string password = "";
    for (int i = 0; i < user_number; i++) {
        std::string passW = words[rand() % words.size()];
        password += passW;
    }
    return password;
}

std::string rep_with_upper(std::string user_string) {
    std::vector<char> chars(user_string.begin(), user_string.end());
    int num = rand() % user_string.length();
    chars[num] = toupper(chars[num]);
    std::string changed_string(chars.begin(), chars.end());
    return changed_string;
}

std::string swap_letters(std::string user_string) {
    std::vector<char> chars(user_string.begin(), user_string.end());
    char a = chars[0];
    char b = chars[1];
    chars[0] = chars[user_string.length() - 2];
    chars[1] = chars[user_string.length() - 1];
    chars[user_string.length() - 2] = a;
    chars[user_string.length() - 1] = b;
    std::string changed_string(chars.begin(), chars.end());
    return changed_string;
}
...
int main() {
    ...
    while (x) {
        if (std::stoi(userNumber) < 3 || std::stoi(userNumber) > 7) {
            std::cout << "Invalid value please try again!\n";
            x = false;
        } else {
            generatedPass = passGen(std::stoi(userNumber));
            std::cout << generatedPass << std::endl;
        }
        x = false;
    }
    std::string upperPass = rep_with_upper(generatedPass);
    std::cout << upperPass << std::endl;
    std::string swapPass = swap_letters(upperPass);
    std::cout << swapPass << std::endl;
    std::cout << "initial password: " << generatedPass << std::endl;
    std::cout << "final password: " << swapPass << std::endl;
    char letter;
    std::cout << "Please enter the letter in the searched password: ";
    std::cin >> letter;
    std::cout << search_letter(swapPass, letter) << std::endl;
    return 0;
}
